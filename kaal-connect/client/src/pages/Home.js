...
const [stats, setStats] = useState({ artists: 0, artworks: 0, collectors: 0 });
const [displayStats, setDisplayStats] = useState({ artists: 0, artworks: 0, collectors: 0 });
...
useEffect(() => {
  ...
  const timer = setInterval(() => {
    ...
    setDisplayStats({
      artists:    Math.floor(stats.artists * ease),
      artworks:   Math.floor(stats.artworks * ease),
      collectors: Math.floor(stats.collectors * ease),
    });
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        <div className="page-container w-full py-20">
          ...
          <div className="flex gap-12 mt-16 pt-10 border-t border-ink-700/50 animate-fade-up" style={{ animationDelay: '0.6s', opacity: 0 }}>
           {[
  { value: displayStats.artists,    label: 'Artists' },
  { value: displayStats.artworks,   label: 'Artworks' },
  { value: displayStats.collectors, label: 'Collectors' },
].map(({ value, label }) => (
  <div key={label}>
    <div className="font-display text-2xl md:text-3xl font-bold" style={{ color: '#7bbbff' }}>
      {value.toLocaleString('en-IN')}
      {value > 0 && <span style={{ color: '#b8a9ff' }}>+</span>}
    </div>
    <div className="text-xs font-sans tracking-widest uppercase mt-1" style={{ color: '#9b8aee' }}>
      {label}
    </div>
  </div>
))}          </div>
        ...
        <div className="page-container">
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-parchment-300/30 font-sans text-sm mb-6">Be the first artist to list your work on KALACONNECT.</p>
